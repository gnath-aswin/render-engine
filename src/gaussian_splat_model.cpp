...
// Color encoded in Spherical harmonics: different colour when viewed from
// different angle
...
GaussianSplatModel::GaussianSplatModel(
  ...
  size_t maxGaussians, size_t stride,
  bool loadAllPoints
  ): 
  path(path),
  ...
  loadFromPLY(this->path, this-> maxGaussians, this->stride, this->loadAllPoints);
  uploadPointToGPU();
  uploadQuadToGPU();
...
void GaussianSplatModel::loadSamplePoints(const std::string &path) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    ...
    g.position = glm::vec3(dist(rng), dist(rng), dist(rng));
    ...
    g.color = glm::vec4(colorDist(rng), colorDist(rng), colorDist(rng), 1.0f);
...
void GaussianSplatModel::loadFromPLY(const std::string &path,
                                     size_t maxGaussiansToLoad, size_t stride, bool loadAllPoints) {
  gaussians.clear();
  ...
  if (!isBinaryLittleEndian) {
    std::cout << "Only binary_little_endian PLY is supported now: " << path
              << std::endl;
    return;
  }
  ...
  if(!loadAllPoints){
    vertexCount = std::min(vertexCount, maxGaussiansToLoad);
  }
  ...
  for (size_t i = 0; i < vertexCount; ++i) {
    ...
    if (!file) {
      std::cout << "Stopped early while reading binary PLY at vertex " << i
                << std::endl;
      break;
    }
    ...
    g.position = glm::vec3(values[ix], values[iy], values[iz]);

    g.rotation =
        glm::vec4(ir0 >= 0 ? values[ir0] : 1.0f, ir1 >= 0 ? values[ir1] : 0.0f,
                  ir2 >= 0 ? values[ir2] : 0.0f, ir3 >= 0 ? values[ir3] : 0.0f);

    float fdc0 = if0 >= 0 ? values[if0] : 0.0f;
    float fdc1 = if1 >= 0 ? values[if1] : 0.0f;
    float fdc2 = if2 >= 0 ? values[if2] : 0.0f;

    g.color = glm::vec4(shToRgb(fdc0), shToRgb(fdc1), shToRgb(fdc2), 1.0f);
    ...
    g.scale = glm::vec3(std::exp(rawS0), std::exp(rawS1), std::exp(rawS2));
  ...
  std::cout << "Loaded binary PLY gaussians: " << gaussians.size() << " from "
            << path << std::endl;

  for (size_t i = 0; i < std::min<size_t>(5, gaussians.size()); ++i) {
    const Gaussian &g = gaussians[i];

    std::cout << "Gaussian " << i << " pos=(" << g.position.x << ", "
              << g.position.y << ", " << g.position.z << ") color=("
              << g.color.r << ", " << g.color.g << ", " << g.color.b
              << ") opacity=" << g.opacity << " scale=(" << g.scale.x << ", "
              << g.scale.y << ", " << g.scale.z << ")" << std::endl;
  }
}
...
void GaussianSplatModel::uploadPointToGPU() {
  ...
  glBufferData(GL_ARRAY_BUFFER,
               ...
               gaussians.data(), GL_DYNAMIC_DRAW);

  // position: vec3
  glEnableVertexAttribArray(0);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, position)));

  // opacity: float
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, opacity)));

  // scale: vec3
  glEnableVertexAttribArray(2);
  glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, scale)));

  // color: vec4
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
...
void GaussianSplatModel::uploadQuadToGPU() {
  ...
  // location 1 = aPos
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, position)));
  glVertexAttribDivisor(1, 1);

  // location 2 = aOpacity
  glEnableVertexAttribArray(2);
  glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, opacity)));
  glVertexAttribDivisor(2, 1);

  // location 3 = aScale
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, scale)));
  glVertexAttribDivisor(3, 1);

  // location 4 = aRotation
  glEnableVertexAttribArray(4);
  glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, rotation)));
  glVertexAttribDivisor(4, 1);

  // location 5 = aColor
  glEnableVertexAttribArray(5);
  glVertexAttribPointer(5, 4, GL_FLOAT, GL_FALSE, sizeof(Gaussian),
                        reinterpret_cast<void *>(offsetof(Gaussian, color)));
...
void GaussianSplatModel::setRenderMode(GaussianRenderMode mode) {
  renderMode = mode;
}

GaussianRenderMode GaussianSplatModel::getRenderMode() const {
  return renderMode;
}

void GaussianSplatModel::draw() const {
  if (gaussians.empty()) {
    std::cout << "Gaussian draw skipped. No gaussians" << std::endl;
    return;
  }

  if (renderMode == GaussianRenderMode::Points) {
    if (pointVAO == 0) {
      std::cout << "Point VAO missing\n";
      return;
    }
    glBindVertexArray(pointVAO);
  ...
  if (renderMode == GaussianRenderMode::Quads ||
      renderMode == GaussianRenderMode::CovarianceEllipses) {
    if (quadVAO == 0) {
      std::cout << "Quad VAO missing\n";
      return;
    }
    glBindVertexArray(quadVAO);
    // Draw quad
...
// Sort to blend the gaussians properly
void GaussianSplatModel::sortByDepth(const glm::mat4 &modelMatrix,
                                     const glm::mat4 &viewMatrix) {
  glm::mat4 modelView = viewMatrix * modelMatrix;

  std::sort(gaussians.begin(), gaussians.end(),
            [&modelView](const Gaussian &a, const Gaussian &b) {
              glm::vec4 av = modelView * glm::vec4(a.position, 1.0f);
              glm::vec4 bv = modelView * glm::vec4(b.position, 1.0f);

              // In OpenGL view space, camera looks along -Z.
              // More negative z = farther away.
              return av.z < bv.z; // far to near
            });

  updateGaussianBuffer();
}

void GaussianSplatModel::reloadAllGaussians(){
  loadAllPoints = true;
  stride = 1;
  gaussians.clear();
  loadFromPLY(
...
void GaussianSplatModel::updateGaussianBuffer() {
  glBindBuffer(GL_ARRAY_BUFFER, gaussianVBO);
...
void GaussianSplatModel::pruneByOpacity(float minOpacity) {
  ...
  updateGaussianBuffer();
  std::cout << "After opacity pruning: " << gaussians.size() << " gaussians\n";
}
...
void GaussianSplatModel::pruneByScale(float maxScale) {
  gaussians.erase(std::remove_if(gaussians.begin(), gaussians.end(),
                                 [maxScale](const Gaussian &g) {
                                   float s =
                                       glm::max(g.scale.x,
                                                glm::max(g.scale.y, g.scale.z));
                                   return s > maxScale;
                                 }),
                  gaussians.end());

  updateGaussianBuffer();
  std::cout << "After scale pruning: " << gaussians.size() << " gaussians\n";
}

void GaussianSplatModel::checkGLError(const std::string &label) {
  GLenum err;
  while ((err = glGetError()) != GL_NO_ERROR) {
    std::cout << "OpenGL error after " << label << ": " << err << std::endl;
  }
}
